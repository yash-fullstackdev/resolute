...
async def get_performance(
    ...
    return {
        "tenant_id": tenant_id,
        "total_pnl_inr": float(row["realised_pnl_inr"]) + float(row["unrealised_pnl_inr"]),
        ...
        "max_drawdown_inr": round(float(row["max_drawdown_inr"]), 2),
        "sharpe_ratio": None,
        "profit_factor": None,
        ...
        "daily": [
            {
                ...
                "trades": r["trades"],
                "cumulative_pnl_inr": float(r["cumulative_pnl_inr"]),
            }
            for r in rows
        ],
    }
